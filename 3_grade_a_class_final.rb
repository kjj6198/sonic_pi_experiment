...
in_thread do
  ...
  live_loop :melody do
    ...
    with_fx :echo, phase: 0.1, decay: 0.6 do
      ...
      play my_loop.look(:melody1).tick(:melody2), release: 0.54, amp: 1.2, pan: rrand(-1, 1) if tempo.tick(:tempo)
      sleep 0.125
    end
  end
  sleep 8
  ...
  live_loop :melody2 do
    with_fx :echo, phase: 0.1, decay: 0.6 do
      use_synth :sine
      tick(:melody1) if tick % 16 == 0
      play my_loop.look(:melody1).tick(:melody2), release: 0.54, amp: 1.2 if tempo.tick(:tempo)
      sleep 0.125
    end
  end
  sleep 8
  live_loop :melody3 do
    with_fx :echo, phase: 0.1, decay: 0.6 do
      use_synth :chiplead
      tick(:melody1) if tick % 16 == 0
      play my_loop.look(:melody1).tick(:melody2), release: 0.1, amp: 0.5 if tempo.tick(:tempo)
      sleep 0.125
    end
  end
end
